...
int main(void)
{
    ...
    fclose(input_1_2);
    fclose(output_1_2);
    // BUBBLE SORT RANDOM VALUES TEST 2
    FILE* input_1_3 = fopen("data/data1.txt", "r"); // input random values from the file
    FILE* output_1_3 = fopen("data/output1.3.txt", "w");  // output file for bubble sort random input
    for(int i = 0; i < n; i++)  // scan data from input file
    {
        fscanf(input_1_3, "%lld", &a[i]);
    }
    fprintf(res, "\nBubble sort random values test 2\n");
    bubble_sort(a, n, count);
    fprintf(res, "Count_cmp = %d\nCount_swap = %d\n", count[0], count[1]);// Print count of swaps and comparisons to res.txt
    ...
    for(int i = 0; i < n; i++) // print data to output file
    {
        fprintf(output_1_3, "%lld\n", a[i]);
    }
    fclose(input_1_3);
    fclose(output_1_3);
    // QUICK SORT RANDOM VALUES TEST 2
    FILE* input_1_4 = fopen("data/data1.txt", "r"); // input random values from the file
    FILE* output_1_4 = fopen("data/output1.4.txt", "w");  // output file for qsort sort random input
    for(int i = 0; i < n; i++)  // scan data from input file
    {
        fscanf(input_1_4, "%lld", &a[i]);
    }
    fprintf(res, "\n\nQuick sort random values test 2\n");
    qsort_alg(a, 0, n, count);
    fprintf(res, "Count_cmp = %d\nCount_swap = %d\n", count[0], count[1]); // Print count of swaps and comparisons to res.txt
